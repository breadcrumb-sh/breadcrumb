...
```bash
bash -c 'echo "<clickhouse><custom_settings_prefixes>SQL_</custom_settings_prefixes></clickhouse>" > /etc/clickhouse-server/config.d/custom.xml && /entrypoint.sh'
```

If you're setting up the template yourself, add this as the **start command** on your ClickHouse service in Railway.
