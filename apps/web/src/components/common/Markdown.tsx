  ...
  a: ({ href, children, ...rest }: React.ComponentProps<"a">) => {
    if (href?.startsWith("#span:")) {
      return (
        <a
          href={href}
          className="inline-flex items-center gap-1 rounded border border-zinc-700 bg-zinc-800/60 px-1.5 py-0.5 text-[12px] font-mono text-blue-400 no-underline hover:bg-zinc-700/60 hover:border-zinc-600 transition-colors cursor-pointer"
          {...rest}
        >
          {children}
        </a>
...
// ── Shared plugins (module-level singleton) ──────────────────────────────────

const codePlugin = createCodePlugin({
  themes: ["github-light", "github-dark"],
});

export const mdPlugins = { code: codePlugin };

// ── Wrapper styling ──────────────────────────────────────────────────────────

const wrapperCls =
  "text-sm text-zinc-300 [&_h1]:text-zinc-100 [&_h2]:text-zinc-100 [&_h3]:text-zinc-100 [&_h4]:text-zinc-100 [&_strong]:text-zinc-200" +
  " [&_h2]:text-sm [&_h2]:font-semibold [&_h2]:mt-4 [&_h2]:mb-2 [&_h3]:text-xs [&_h3]:font-semibold [&_h3]:mt-3 [&_h3]:mb-1.5" +
  " [&_p]:leading-relaxed [&_p]:my-1.5" +
  " [&_ul]:my-1.5 [&_ol]:my-1.5 [&_li]:leading-relaxed [&_li]:my-0.5" +
  " [&_a]:text-blue-400 [&_a]:no-underline [&_a:hover]:underline [&_a]:cursor-pointer" +
  " first:[&_h2]:mt-0 first:[&_h3]:mt-0" +
  " [&_hr]:border-zinc-800/40 [&_hr]:my-6";
...
export function Markdown({
  ...
  return (
    <div className={className ? `${wrapperCls} ${className}` : wrapperCls}>
      <Streamdown
